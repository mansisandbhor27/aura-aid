...
import { useDemoDonations } from '../../hooks/useDemoDonations.tsx';
import type { ActivityItem } from '../../types/index.ts';
import { ActivityList } from './ActivityList.tsx';
import { SectionHeading } from '../common/ui.tsx';

export function ActivityFeed({ items }: { items: ActivityItem[] }) {
  const { receipts } = useDemoDonations();
  const [filter, setFilter] = useState<'all' | ActivityItem['kind'] | 'demo'>('all');
  const [loading, setLoading] = useState(true);
  useEffect(() => { const t = window.setTimeout(() => setLoading(false), 500); return () => window.clearTimeout(t); }, []);
  const demoItems: ActivityItem[] = receipts.map((r) => ({
    id: r.receiptId, kind: 'donation',
    title: `${r.privacyMode === 'shielded' ? 'Shielded' : 'Public'} demo donation to ${r.campaignTitle}`,
    detail: `Local demo receipt ${r.receiptId}. Not submitted on-chain.`,
    timestampLabel: r.createdAtLabel, shielded: r.privacyMode === 'shielded',
    amountLabel: r.shieldAmount ? 'Hidden - demo pooled' : `$${r.amount} (demo public)`,
    proofHash: r.proofHash, actorLabel: r.donorLabel ?? 'Anonymous donor (demo)',
  }));
  const combined = [...demoItems, ...items];
  const shown = filter === 'all' ? combined : filter === 'demo' ? demoItems : combined.filter((a) => a.kind === filter);
  return (
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6" aria-labelledby="transparency-heading">
      ...
      <div className="mt-5 flex flex-wrap gap-2" role="group" aria-label="Filter activity">
        {([['all', 'All'], ['donation', 'Donations'], ['milestone-release', 'Milestones'], ['proof-published', 'Proofs'], ['demo', 'My demo session']] as const).map(([id, label]) => (
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="h-fit space-y-3 rounded-3xl border border-white/10 bg-white/[0.03] p-5 lg:sticky lg:top-20">
          ...
          <p className="rounded-xl border border-white/10 p-3 text-xs text-slate-400">Demo placeholders only. No fake explorer links. Hashes become real after indexer integration.</p>
        </div>
        <ActivityList items={shown} loading={loading} demoCount={demoItems.length} onClearFilter={() => setFilter('all')} />
      </div>
    </section>
  );
}
